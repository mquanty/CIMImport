    ...
    public static class CIMModelLoader
    {
        public static CIMModelLoaderResult LoadCIMXMLModel(Stream stream, string fileName, out CIMModel model, string enumMappingFilePath = "")
        {
            ...
            try
            {
                ...
                handler = (CIMXMLReaderHandler)XMLParser.DoParse(handler, stream, fileName, out bool success, out durationOfParsing);
        ...
        public static CIMModelLoaderResult LoadCIMModelForDifference(ref CIMDifference cimDifference)
        {
            ...
            if (cimDifference != null)
            {
                try
                {
                    ...
                    handler = (CIMRDFComparerXMLHandler)XMLParsingManager.DoParse(handler, XMLParsingManager.ParsingFileTypes.CIMModelFile, cimDifference.Extract, out bool success, out TimeSpan durationOfParsing);
                    if (success)
                    {
        ...
        public static void LoadCIMObject(ref CIMObject cimObject, CIMModelContext cimModelContext, string text)
        {
            if (!string.IsNullOrEmpty(text))
            {
                ...
                //// call parsing: 
                handler = (CIMRDFObjectXMLHandler)XMLParsingManager.DoParseText(handler, XMLParsingManager.ParsingFileTypes.CIMModelFile, text, out bool success, out TimeSpan durationOfParsing);
