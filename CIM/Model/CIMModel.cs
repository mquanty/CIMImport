...
namespace CIM.Model
{
    ...
    public class CIMModel
    {
        ...
        private List<string> cimrdfids; //ADDED BY MKM TO STORE THE SEQUENCE OF RDF ID INSERTED 
                                        //// structure that keeps all found CIM types, attributes...
        private readonly CIMModelContext modelContext = new CIMModelContext();
        ...
        /// </summary>
        public SortedDictionary<string, SortedDictionary<string, CIMObject>> ModelMap => modelMap;

        /// <summary>
        /// Gets the CIM model context instance of this model.
        /// </summary>
        public CIMModelContext ModelContext => modelContext;
        ...
        /// </summary>
        public double FileSizeMB => fileSizeMB;
        ...
        /// </summary>
        public Dictionary<string, List<string>> NodeToElementMap => nodeToElementMap;
        ...
        /// </summary>
        public Dictionary<string, List<string>> ElementToNodeMap => elementToNodeMap;

        /// <summary>
        /// Gets the map with identifiers of all PowerTransformer CIMObjects 
        /// grouped by their voltage level (HV, HV/MV, LV).
        /// </summary>
        public Dictionary<CIMConstants.VoltageLevel, List<string>> VoltageLevelMap => voltageLevelMap;
        ...
        /// <summary>added by MKM</summary>
        public List<string> GetAllRDFIDs()
        {
            return cimrdfids;
        }
        /// <summary>added by MKM</summary>
        public string GetNextRDFID(string rdfid)
        {
            if (cimrdfids == null) return null;
            int index = cimrdfids.IndexOf(rdfid); //if not found will return -1
            if (index >= 0 && index < cimrdfids.Count - 1) //index is 0 based and will end in count-1, so our last valid input is count -2
                return cimrdfids[index + 1];
            return null;
        }
        /// <summary>added by MKM</summary>
        public CIMObject GetNextObject(string rdfid)
        {
            string nextobjid = GetNextRDFID(rdfid);
            if (nextobjid == null) return null;

            CIMObject findobj = GetObjectByID(nextobjid);
            return findobj;
        }
        ...
        public void InsertObjectInModelMap(CIMObject cimObject)
        {
            if ((cimObject != null) && !string.IsNullOrEmpty(cimObject.ID) && !string.IsNullOrEmpty(cimObject.CIMType))
            {
                ...
                //added by MKM
                if (cimrdfids == null)
                    cimrdfids = new List<string>();
                cimrdfids.Add(cimObject.ID); //store all rdf ids
            }
        }
        ...
        public List<string> FindAllEnergySources()
        {
            ...
            if ((modelMap != null) && (modelMap.ContainsKey(CIMConstants.TypeNameTransformerWinding)))
            {
                ...
                modelMap.TryGetValue(CIMConstants.TypeNameTransformerWinding, out SortedDictionary<string, CIMObject> allTransformerWindings);
                            ...
                            if ((voltage >= CIMConstants.VoltageRangeLowerKV) && (voltage <= CIMConstants.VoltageRangeUpperKV))
                            {
                                ...
                                else
                                {
                                    ...
                                    if ((pt != null) && (pt.Count > 0))
                                    {
                                        ...
                                        if (ptObject != null)
                                        {
                                            Dictionary<CIMObject, CIMConstants.WindingType> analizedWindings = AnalyzeWindingsOfPowerTransformer(ptObject, out double ptVoltageValue);
        ...
        public CIMConstants.VoltageLevel GetVoltageLevelOfPowerTransformer(CIMObject powerTransformer)
        {
            ...
            if ((powerTransformer != null) && (string.Compare(CIMConstants.TypeNamePowerTransformer, powerTransformer.CIMType) == 0))
            {
                if (voltageLevelMap != null)
                {
                    ...
                    {
                        voltageLevelMap.TryGetValue(voltageL, out List<string> ptIdentifiers);
        ...
        private void AddToElementToNodeMap(string elementId, string connectivityNodeId)
        {
            ...
            elementToNodeMap.TryGetValue(elementId, out List<string> eToN);
            if (eToN == null)
            {
        ...
        private void SortTransformerWindingsOfModel()
        {
            if (modelMap != null)
            {
                ...
                if (allPowerTransformers != null)
                {
                    ...
                    {
                        Dictionary<CIMObject, CIMConstants.WindingType> analized = AnalyzeWindingsOfPowerTransformer(pt, out double ptVoltageValue);
                        if (analized.Count > 0)
                        {
