    ...
    internal class RDFSXMLReaderHandler : IHandler
    {
            ...
            if (!abort)
            {
                ...
                else if (qName.Equals(cimsIsAggregate)) //// end of isAggregate subelement
                {
                    ...
                    if (!string.IsNullOrEmpty(content))
                    {
                        if (currentElement != null)
                        {
                            if (bool.TryParse((string)content.Clone(), out bool paresedValue))
                            {
        ...
        private void ProcessProfile()
        {
            if (profile.ProfileMap != null)
            {
                ...
                {
                    profile.ProfileMap.TryGetValue(ProfileElementTypes.Unknown, out List<ProfileElement> unknownsList);
                    profile.ProfileMap.TryGetValue(ProfileElementTypes.EnumerationElement, out List<ProfileElement> enumerationElementsList);
